...
public class TransferFunction {
    ...
    public double gradientWeighting(double fxi){
        
            double alpha = 0;
            for (int j = 1; j < controlPoints.size() - 1; j++) {
                double Fvn = controlPoints.get(j).value; //the value where we want it to be exactly alphaV
                double Fvn1 = controlPoints.get(j+1).value;
                double alphaVn = controlPoints.get(j).color.a;
                double alphaVn1 = controlPoints.get(j+1).color.a;
                
                if( fxi >= Fvn && fxi <= Fvn1) {
                    alpha = alphaVn1*((fxi - Fvn)/(Fvn1 - Fvn))+ alphaVn*((Fvn1-fxi)/(Fvn1-Fvn));
                    return alpha;
                }
                else  
                    alpha = 0;
